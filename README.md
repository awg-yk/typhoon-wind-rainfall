...
- `scripts/download_jra3q_pressure.py` … ERA5との精度比較のため、気象庁
  ...
  **速度より確実さを優先した設計で、全185ヶ月で1時間程度かかります。** GDEXの
  サーバーは負荷に弱く、少しでも重いリクエストを送るとnginxが
  「504 Gateway Time-out」を返してくるため:
  - 既定で**逐次実行**（並列にすると大半が504で失敗）
  - 1ヶ月分（124時刻）を一度に要求せず、**5日ずつに分割**して取得
    （`--chunk-days` で変更可。504が多発するなら `2` などに下げる）
  - 各リクエストは最大5回、待ち時間を倍々にしながら再試行
  - さらに月単位でも最大3周まで再試行
