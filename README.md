...
被害件数（`deathCount()`/`damageScore()`、index.html内）に基づきます。

死者・行方不明者は1人でもいれば、件数の多寡によらず**最も濃い色（5段階
中の最上位）**にします。件数の桁が違いすぎるため（死者数人 vs. 床下浸水
数千棟など）、死者数を他の被害と合算してしまうと、死者ゼロで浸水が
多い県のほうが死者ありの県より濃い色になってしまうことがあるためです。

死者・行方不明者が0人の都道府県どうしは、残り4段階の中で加重スコアに
基づき比較・塗り分けします。負傷者は全壊・半壊などの建物被害や
床上・床下浸水より件数が少ない一方で影響が大きいため、次の重みで
加算しています。
