...
被害件数に基づく加重スコア（`damageScore()`、index.html内）で、単純合算
ではなく人的被害を重視した重み付けをしています。死者・行方不明者・負傷者
は全壊・半壊などの建物被害や床上・床下浸水より件数が少ない一方で影響が
大きいため、次の重みで加算しています。

| 区分 | 例 | 重み |
| --- | --- | --- |
| 人的被害 | 死者・行方不明者、死者、行方不明者 | ×100 |
| 人的被害 | 負傷者 | ×20 |
| 建物の全損 | 全壊、流失、焼失 | ×5 |
| 建物の半損 | 半壊、半焼 | ×2 |
| 軽微な被害 | 一部破損、床上浸水 | ×1 |
...
その台風の中で最も被害スコアが大きかった都道府県を基準に5段階で
塗り分けています（時系列ではなく、台風ごとの最終的な被害の比較です）。
都道府県にカーソルを合わせると、死者・行方不明者、負傷者、全壊などの
実数の内訳がツールチップで表示されます（色分けは加重スコア基準です
が、ツールチップは加工していない実数です）。
